...
This can be useful when e.g. modeling valves.
...
Considering the previous example depicted {ref}`flow_sheet_splitter` and assuming a flow rate of $1~mL\cdot s^{-1}$, the flow rate is evenly split:
