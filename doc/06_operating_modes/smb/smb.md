...
# Import the flip_flop module
smb_path = diss_root / "studies" / "smb"
sys.path.insert(0, str(smb_path))
from smb.process import setup_process, plot_results
```
...
```{figure} ./figures/smb_flow_sheet.png
:name: smb_system
...
smb_builder = setup_smb_builder(switch_time=100)
smb_builder.column.solution_recorder.write_solution_bulk = True
Q_feed, Q_eluent, w_r, w_e = smb_builder.triangle_design(gamma=1.1)
...
fig_bulk, _ = plot_bulk(smb_builder, simulation_results)
glue("smb_bulk", fig_bulk, display=False)
fig_outlets, _ = plot_outlets(simulation_results)
glue("smb_outlets", fig_outlets, display=False)
```
