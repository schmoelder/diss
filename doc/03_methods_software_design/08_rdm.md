...
Reproducibility is compromised when code versions are not tracked, configurations are insufficiently documented, or results are recorded while the code or analysis pipeline that produced them is still evolving.
...
To address these challenges, the CADET-RDM framework was developed in collaboration with Ronald Jäpel; the workflows presented in this thesis served as its primary design application, and all case studies were managed using it {cite}`CADET-RDM_documentation`.
...
In this work, CADET-RDM provides the infrastructure required to systematically link simulations, parameter studies, and optimization results to their exact computational context, ensuring that all reported results remain reproducible and fully traceable.
